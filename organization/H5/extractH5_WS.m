...
% NOT WORKING April 2026: data.StimulusLibrary = sub_orgStimLib(tmpStimLib); %Call a function to reorganize the stimulus library
%into a new format
...
nSweeps = size(h5Struct.Groups,1) - 1;
...
nDIChan = data.header.NDIChannels;  nAIChan = data.header.NAIChannels;
data.header.nTraces = nDIChan + nAIChan;
if nDIChan > 0
    data.header.DIStatus = 1;
else
    data.header.DIStatus = 0;
end
if nAIChan > 0
    data.header.AIStatus = 1;
else
    data.header.AIStatus = 0;
end
data.allNames = {};
data.AINames = data.header.AIChannelNames;
data.allNames = {data.allNames{:},data.header.AIChannelNames{:}};
data.DINames = data.header.DIChannelNames;
data.allNames = {data.allNames{:},data.header.DIChannelNames{:}};
for n = 1:nSweeps
    rootVal = h5Struct.Groups(n+1).Name;
    data.sweeps(n).allData = [];
    if data.header.AIStatus == 1
        analogData = h5read(h5file,strcat(rootVal,'/analogScans'));
        scaledData = scaleData_WS(analogData,channelScale,scalingCoeff);
        data.sweeps(n).AIData = scaledData;
        data.sweeps(n).time = (1:length(scaledData))'/sampleRate;
        data.sweeps(n).allData = [data.sweeps(n).allData,scaledData];
    end
    if data.header.DIStatus == 1
        digitalData = h5read(h5file,strcat(rootVal,'/digitalScans'));
        for m = 1:nDIChan
            data.sweeps(n).DIData(:,m) = bitget(digitalData,m);
        end
        data.sweeps(n).allData = [data.sweeps(n).allData,double(data.sweeps(n).DIData)];
    end
end
end
% -------------------------------------------------------------------------------------------------
